...
def save_txt():
    com = pd.read_csv("./data/去除停用词并分词.csv", encoding='gb18030')
    for line in com.values:
        with open('./data/去除停用词并分词.txt', 'a+', encoding='gb18030') as f:
            f.write((str(line[0])))
...
if __name__ == '__main__':
    ...
    data.columns = ['评论']
    # 存储数据
    data.to_csv('./data/评论去重.csv', encoding='gb18030', index=False)

    # # 去除换行符
    # path = '评论去重.csv'
    # with open(path, encoding='gb18030', newline='') as fin:
    #     with open('评论去换行.csv', 'w', encoding='gb18030', newline='') as fon:
    ...
    # 去除停用词并jieba分词
    data = pd.read_csv('./data/评论去重.csv', encoding='gb18030').astype(str)
    ...
    df.to_csv("./data/去除停用词并分词.csv", encoding='gb18030', index=False)

    # 去除空行
    find_line = pd.read_csv("./data/去除停用词并分词.csv", encoding='gb18030')
    find_line.dropna(axis=0, how='any', inplace=True)
    find_line.to_csv('./data/去除停用词并分词.csv', encoding='gb18030', index=False)
